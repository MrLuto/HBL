...
Function GetCalendars
{
 ...
 {
  Get-MailboxFolderStatistics -identity $EmailAddress | Where-Object{$_.FolderType -eq 'Calendar'} | select -expand name -OutVariable CalendarFolderLanguageName | Out-Null

  if (($LiveCalendarFolder.FolderType) -eq "Calendar")
  {
  $CurrCalendarFolder = $EmailAddress + ":\" + $CalendarFolderLanguageName
  }
  else
  {
  $CurrCalendarFolder = $EmailAddress + ":\" + $CalendarFolderLanguageName + "\" + $LiveCalendarFolder.Name
  }
