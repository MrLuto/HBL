...
Release Notes:
    ...
    v2.6 - Update to use ProhibitSendReceiveQuota value for calculation
Sample Usage:
    Execute script using MFA account
        ./GetMailboxSizeReport.ps1 -MFA
    
    Execute script with non-MFA account.
...
Function Get_MailboxSize
{
 ...
 $TotalDeletedItemSizeinBytes           = $Stats.TotalDeletedItemSize -replace "(.*\()|,| [a-z]*\)", ""

 $ProhibitSendReceiveQuotainBytes       = $Mailbox.ProhibitSendReceiveQuota -replace "(.*\()|,| [a-z]*\)", ""

 if ( ($TotalItemSizeinBytes -gt 0) -and ($SystemMessageSizeShutoffQuotainBytes -gt 0) ) {
    $PercentageUsed                     = ( [int64]$TotalItemSizeinBytes / [int64]$ProhibitSendReceiveQuotainBytes ) * 100
 } else {
    $PercentageUsed                     = 0
 }
 $PercentageUsed                        = [math]::Round($PercentageUsed,3)
 ...
 $Result = @{
    ...
    'Percentage Used (Calculated)'  = $PercentageUsed
}
 $Results= New-Object PSObject -Property $Result  
 $Results | Select-Object 	'Display Name', `
                            'User Principal Name', `
                            'Mailbox Type', `
...
Function main()
{
 ...
 { 
  ...
  foreach($item in $Mailboxes)
  {
   ...
   $ProhibitSendReceiveQuotainBytes     = $MBDetails.ProhibitSendReceiveQuota -replace "(.*\()|,| [a-z]*\)", ""
